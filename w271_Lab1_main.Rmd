...
knitr::kable(
  admissions[1:5,1:5 ], caption = 'Top 6 Rows of Admissions Dataset')
```
...
```{r}
challenger2 <- read.table("challenger.csv", header=TRUE, sep=",")
...
w.n <- data.frame(Temperature=w$Temp, success=w$O.ring, trials=n$O.ring, proportion = round(w$O.ring / n$O.ring, 4))
...
K <- matrix(data = c(31), nrow=1, ncol=1)
