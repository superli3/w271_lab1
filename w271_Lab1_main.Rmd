...
p = ggplot() +
  #geom_point(data = w.n, aes(x = Temperature, y = w$O.ring / n$O.ring), size=1) +
...
simulate <- function(){
  ...
  mod.fit <- suppressWarnings(glm(bootstrap_y ~ bootstrap_x, family = binomial(link=logit), maxit = 100))
  beta0.bs = mod.fit$coefficients[1]
  beta1.bs = mod.fit$coefficients[2]
