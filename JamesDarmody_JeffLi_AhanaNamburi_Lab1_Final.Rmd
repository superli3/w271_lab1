...
grid.arrange(hist.pressure, hist.temp, ncol=2)

```

- Pressure: The histogram above confirms our notion that PSI measurements in the dataset occur in regular increments, with most measurements taking place at 200psi, but other measurements also taking place at 50 and 100 psi as well. We had considered converting Pressure to a categorical variable, since there are only 3 distinct values for Pressure (50, 100, 200) in the given dataset. However, we were also concerned that in converting Pressure to categorical, we would be losing some of the scaling factor of Pressure, thereby losing some of the predictive power for the variable. We choose to leave as a discrete integer variable.

- Temperature: We extend our histogram analysis further, this time examining temperature.  The distribution has two major peaks, right around upper 60s, and mid 70s, with only a few observations warmer or cooler than those areas.  This is a continuous variable.
...
plot1 <- ggplot(challenger, aes(factor(O.ring), Temp)) +  
  geom_boxplot(aes(fill = factor(O.ring))) + 
  ggtitle("Temperature by O.ring Failure") + 
  theme(plot.title = element_text(lineheight=1, face="bold"))

plot2 <- ggplot(challenger, aes(factor(O.ring), Pressure)) +  
  geom_boxplot(aes(fill = factor(O.ring))) + 
...
O.ring_binomial <- glm(formula=cbind(O.ring,6-O.ring) ~ 
                      Temp + Pressure, family=binomial(link="logit"),
...
stargazer(O.ring,O.ring2,O.ring3, O.ring4, header = F, type = 'latex', omit.table.layout= 'n',
          title='Regression Results for Models to check significance of Pressure')

```

We have considered 4 models:

- Model1 - Logistic regression model with Temperature and Pressure
- Model2 - Logistic regression model with only Temperature
- Model3 - Logistic regression model including an interaction term between Pressure and Temperature
- Model4 - Logistic regression model incluing a squared Pressure term
...
#Odds Ratios
c=-1
exp(c*O.ring.final$coefficients[2])
```
...
We have chosen our Final model to be the **Logistic Regression Model with only Temperature as the indicator variable**. Our dataset has a binary response variable, which is 1 if there is a failure and 0 if there is none. 

- **Model Evaluation** : We evaluated linear models and other logistic regression models using pressure term, squared and interaction terms to find a better fit. In the liner models we do not meet the CLM conditions, while in the logistic regression models we do not observe any significant terms through the model evaluation. Our model specification is as provided below:

$$
logit(\hat{\pi}) =  15.043 - 0.232(Temp)
$$

- **Confidence Intervals** : We also believe that a smaller dataset has led to larger/wider confidence intervals at lower temperatures versus higher temperatures. We are constrained by the the nature of the data as there are very limited space shuttle launches. 

- **Odds Ratios** : Based on the above, we observe that the odds of success (Failure of O-Ring) change by 1.261 times over for every degree change in temperature. Lower Temperatures increase the probability of failure of O-Ring. We also observe this in graph generated in part 3b, where the probability of failure consistently drops with an increase in temperature. Given our lowest observed temperature is 53 degrees in the dataset, we observe that probability of O-ring failure is .9996 at 31 degrees which is also the temperature at which Challenger was launched. Based on this data, we would not recommend that Challenger be launched when the temperature was 31 degrees.
